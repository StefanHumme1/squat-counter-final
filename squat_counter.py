...
# the keypoints are stored in a tensor keypoint array for example the x value
# for the first keypoint 0 is stored in [0] the y value in [1] and the confidence value
# in [2] for the second keypoint 1 the x value is stored in [3] and so on
# is the confidence value (conf) under 0.5 its likely that this keypoint is not sufficient enough
# detect and it gets the value 0. All the 17 keypoints are stored in an array named keypoints
def get_all_keypoints(kpts):
    keypoints = []
    steps = 3
    for i in range(17):
        x, y, conf = kpts[steps * i], kpts[steps * i + 1], kpts[steps * i + 2]
        if conf < 0.5:
            x = 0
            y = 0
        x = float(x) if isinstance(x, (int, float)) else float(x.item())
        y = float(y) if isinstance(y, (int, float)) else float(y.item())
        keypoints.append((x, y))
    return keypoints

# here we build the header for our json file and the keypoint data is later
# stored in the "Frames" array
recordingID = str(datetime.now()) + str(random.randint(0, 100))
data = {
  "RecordingID": recordingID,
  "ApplicationName": "YOLOv7-Pose",
  "OpenName": None,
  "Frames": []
}
...
# here we store the collected keypoint data into a json file
# the indent command supports the readability
...
def get_coords(kpts):
    ...
    # Face
    face = [2]
    coords2 = get_hip_knee(kpts, face)
    # print(coords2)
    
    # Get all 17 Keypoints (X and Y Values) and set the values for each variable
    keypoints = get_all_keypoints(kpts)
    nose_x = keypoints[0][0]
    nose_y = keypoints[0][1]
    rightEye_x = keypoints[1][0]
    rightEye_y = keypoints[1][1]
    leftEye_x = keypoints[2][0]
    leftEye_y = keypoints[2][1]
    rightEar_x = keypoints[3][0]
    rightEar_y = keypoints[3][1]
    leftEar_x = keypoints[4][0]
    leftEar_y = keypoints[4][1]
    rightShoulder_x = keypoints[5][0]
    rightShoulder_y = keypoints[5][1]
    leftShoulder_x = keypoints[6][0]
    leftShoulder_y = keypoints[6][1]
    rightArm_x = keypoints[7][0]
    rightArm_y = keypoints[7][1]
    leftArm_x = keypoints[8][0]
    leftArm_y = keypoints[8][1]
    rightHand_x = keypoints[9][0]
    rightHand_y = keypoints[9][1]
    leftHand_x = keypoints[10][0]
    leftHand_y = keypoints[10][1]
    rightThigh_x = keypoints[11][0]
    rightThigh_y = keypoints[11][1]
    leftThigh_x = keypoints[12][0]
    leftThigh_y = keypoints[12][1]
    rightKnee_x = keypoints[13][0]
    rightKnee_y = keypoints[13][1]
    leftKnee_x = keypoints[14][0]
    leftKnee_y = keypoints[14][1]
    rightFoot_x = keypoints[15][0]
    rightFoot_y = keypoints[15][1]
    leftFoot_x = keypoints[16][0]
    leftFoot_y = keypoints[16][1]
    
    # when the program is in this loop than the measuring of the time is starting
    # to get exact timestamps for each captured frame
    global start_time
    ...
    frameStamp = f"{int(hours):02d}:{int(minutes):02d}:{int(seconds):02d}.{int(milliseconds):03d}"
    ...
    data["Frames"].append({
        ...
        "frameAttributes": {
            ...
            "rightArm_x": rightArm_x,
            "rightArm_y": rightArm_y,
            "leftArm_x": leftArm_x,
            "leftArm_y": leftArm_y,
            "rightHand_x": rightHand_x,
            "rightHand_y": rightHand_y,
            "leftHand_x": leftHand_x,
            "leftHand_y": leftHand_y,
            "rightThigh_x": rightThigh_x,
            "rightThigh_y": rightThigh_y,
            "leftThigh_x": leftThigh_x,
            "leftThigh_y": leftThigh_y,
            "rightKnee_x": rightKnee_x,
            "rightKnee_y": rightKnee_y,
            "leftKnee_x": leftKnee_x,
            "leftKnee_y": leftKnee_y,
            "rightFoot_x": rightFoot_x,
            "rightFoot_y": rightFoot_y,
            "leftFoot_x": leftFoot_x,
    ...
    # Hands
    left_hand = [10] # keypoint for the left hand
    right_hand = [9] # keypoint for the right hand
    coordsLeftHand = get_hip_knee(kpts, left_hand) # Koordinaten der linken Hand
    coordsRightHand = get_hip_knee(kpts, right_hand) # Koordinaten der rechten Hand
    """
    if((len(coordsLeftHand) > 0) & (len(coordsRightHand) > 0)): # Das Array mit den beiden Keypoints der Hände sollte Daten beinhalten
      if coordsRightHand[0][0].item() < coordsLeftHand[0][0].item(): # [0][0].item() liefert die jeweilige X-Koordinate und [1][0].item() liefert die jeweilige Y-Koordinate
        print("Arme sind verschlossen")
    else:
        print("Arme sind offen")
    """
